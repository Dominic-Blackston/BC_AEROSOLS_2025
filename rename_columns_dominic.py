...
campaign = "CALNEX 2010"  # Change campaign name if needed.
...
df_raw["Organization"] = "NSF"   # (or NOAA, DOE, etc.)
...
aerosol_bin_candidates = [
    ...
    lambda col: col.startswith("UHSAS_") and col.endswith("nm_PSL"),
    lambda col: col.startswith("UHSAS_") and col.endswith("nm"),
    lambda col: col.startswith("0.") and col.endswith(""),
    lambda col: col.startswith("dNdlogDp_PSL_") and col.endswith("nm_LAS_LARGE"),
    lambda col: col.startswith("dNdlogDp_PSL_") and col.endswith("nm_LAS_LARGE"),
    # lambda col: col.startswith("1") and col.endswith("nm"),
    # lambda col: col.startswith("2") and col.endswith("nm"),
    # lambda col: col.startswith("3") and col.endswith("nm"),
    # lambda col: col.startswith("4") and col.endswith("nm"),
    # lambda col: col.startswith("5") and col.endswith("nm"),
    # lambda col: col.startswith("6") and col.endswith("nm"),
    # lambda col: col.startswith("7") and col.endswith("nm"),
    # lambda col: col.startswith("8") and col.endswith("nm"),
    # lambda col: col.startswith("9") and col.endswith("nm"),
...
cloud_bin_candidates = [
    ...
    lambda col: col.startswith("C:") and col.endswith(""),
...
restricted_expected = {
    ...
    "Date": ["Date", "date"],
    "Latitude": ["LATITUDE", "GPS_LAT", "Lat", "FMS_LAT", "G_LAT"],
    "Longitude": ["LONGITUDE", "GPS_LON", "Lon", "FMS_LON", "G_LONG"],
    "Altitude": ["GPS_ALT", "Alt", "FMS_ALT_PRES", "GPS_Altitude", "MSL_GPS_Altitude", "G_ALT"],
    "Temperature": ["Static_Air_Temp", "T", "C_StatTempDegC", "Temperature"],
    "Rel_humidity": ["RH_amb", "Relative_Humidity", "RHamb", "fRH550_RH20to80", "Humidity"],
    "Pressure": ["P", "Static_Pressure", "FMS_ALT_PRES", "C_StaticPressure", "Pressure"],
    "BC_Mass": ["BlackCarbonMassConcentration", "BC_Mass", "mBC", "BC_AccumMode_mass_HDSP2", "BC_mass_90_550_nm_HDSP2", "BlackCarbon_STP", "BC_mass_90_550_nm", "SP2_rBC_conc"],
    "Sc450_total": ["totSc450_stdPT", "Sc450_total", "Scat450tot", "drySc450_stdPT", "Scat_blue_dry_TSI3563_LARGE", "Bs_B"],
    "Sc550_total": ["totSc550_stdPT", "Sc550_total", "Scat550tot", "drySc550_stdPT", "Scat_green_dry_TSI3563_LARGE", "Bs_G"],
    "Sc700_total": ["totSc700_stdPT", "Sc700_total", "Scat700tot", "drySc700_stdPT", "Scat_red_dry_TSI3563_LARGE", "Bs_R"],
    "Abs470_total": ["absSc470_stdPT", "Abs470_total", "Abs470tot", "Abs470_stdPT", "Abs_blue_dry_PSAP_LARGE", "Ba_B"],
    "Abs532_total": ["absSc532_stdPT", "Abs532_total", "Abs532tot", "Abs532_stdPT", "Abs_green_dry_PSAP_LARGE", "Ba_G"],
    "Abs660_total": ["absSc660_stdPT", "Abs660_total", "Abs660tot", "Abs660_stdPT", "Abs_red_dry_PSAP_LARGE", "Ba_R"],
}

# For the Comprehensive file: include everything in restricted plus additional variables.
comprehensive_expected = {
    **restricted_expected,
    "U": ["U", "N/S Wind Speed", "U_ms-1", "u_w"],
    "V": ["V", "E/W Wind Speed", "V_ms-1", "v_w"],
    "W": ["W", "Vertical Wind Speed", "w_ms-1", "w_w"],
    "Supersaturation": ["Supersaturation", "CCN_Supersaturation"],
    "Number_Concentration": ["Number_Concentration"],
    "CNgt3nm": ["CNgt3nm", "CN>3nm", "CPC_Conc_3025"],
    "CNgt10nm": ["CNgt10nm", "CN>10nm", "CPC_Conc_3010"],
