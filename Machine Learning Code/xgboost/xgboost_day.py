...
for no_bins in bins:
    for split_strat in split_strats:
        print(f"Running xgboost_day.py with {no_bins} bins and split strategy {split_strat}")
        if no_bins == 3.1:
            DATASET_PATH = rf"split_datasets/data_split_strat{split_strat}/MAC_DATASET_LLOD_FILTERED/day_based_split"
            OUTPUT_PATH = rf"outputs/output_xgboost_day_{no_bins}bins_{split_strat}splitstrat_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        else:
            # SET YOUR DATASET PATH HERE
            DATASET_PATH = rf"split_datasets/data_split_strat{split_strat}/MAC_binning_{no_bins}bins_optimized/day_based_split"
            OUTPUT_PATH = rf"outputs/output_xgboost_day_{no_bins}bins_{split_strat}splitstrat_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        print("="*70)
        print("XGBOOST DAY-BASED SPLIT WITH COMPLETE PIPELINE")
        print("="*70)
        ...
        with open(os.path.join(OUTPUT_PATH, "day_based_complete_report.txt"), 'w') as f:
            f.write("XGBOOST DAY-BASED SPLIT COMPLETE PIPELINE REPORT\n")
            f.write("="*70 + "\n")
        ...
        print("FILES CREATED:")
        print("="*60)
        print("- final_xgboost_day_model.pkl (trained model)")
        ...
        print("- day_based_complete_report.txt (comprehensive report)")

        print("\n" + "="*70)
        print("DAY-BASED COMPLETE PIPELINE COMPLETED SUCCESSFULLY!")
        print("="*70)
        print("\nSUMMARY:")
        ...
        print("2. Ensure your day-based split CSV files are in the correct directory")
